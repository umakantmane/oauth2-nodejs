...
function extractCredentials (done) {
 ...
 // this.grantType = 'password';
   //return done(error( this.grantType));
	//done(!this.grantType);
 ...
 */
 ...
 /**
 *
 static secret key and clientId
 if you need dynamically then supply these keys by form level and receive here
 Client(req.boby.client_id, req.body.secret_key); 
 *
 */
 
function credsFromBody (req) {
  return new Client('s6BhdRkqt3', 'umakant');  
}
...
function usePasswordGrant (done) {
  ...
  return this.model.getUser(uname, pword, function (err, user) {
    if (err) return done(error('server_error1', false, err));
    if (!user) {
